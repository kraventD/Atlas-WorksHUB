...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  if (existing) {
    ...
    await awardXp(supabase, session.user.id, -XP_REWARDS.COLLECTION)
    return new Response(JSON.stringify({ collected: false }), { headers: { "content-type": "application/json" } })
  }

  await supabase.from("collection").insert({ user_id: session.user.id, game_slug })
  ...
  return new Response(JSON.stringify({ collected: true }), { headers: { "content-type": "application/json" } })
}
